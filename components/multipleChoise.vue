<template>
  <v-container>
    <v-row>
      <v-card elevation="2" outlined shaped class="rounded-xl">
    ...
    <v-row>
      <v-col v-for="(item, i) in options" :key="i">
        <v-hover v-slot:default="{ hover }" open-delay="200">
          ...
          >
            <v-card-text class="d-flex justify-center"> {{ item.text }}</v-card-text>
    ...
    <v-row>
      <v-card @click="nextQuestion" v-if="selected" outlined shaped class="rounded-xl">
        <v-card-text class="d-flex justify-center text-center">
